...
class UserService extends Service {
  async findAll(payload) {
    ...
    !ctx.helper.tools.isParam(department_id)
      ? (where.department_id = department_id === 0 ? null : department_id)
      : null;
  ...
  async login(payload) {
    const { ctx, app } = this;
    const user = await ctx.model.Users.scope('withPassword').findOne({
    ...
    payload = Object.assign(
      payload,
      await ctx.helper.tools.saltPassword(payload.password, user.dataValues.password.substr(32)),
    );
    payload.password += payload.salt;
  ...
  async updateUserDepartment(payload) {
    ...
    return await ctx.model.Users.update(
  ...
  async refreshToken(payload) {
    ...
    try {
      ...
      try {
        ...
        if (
          currentRequestData.userInfo &&
          currentRequestData.userInfo.id &&
          currentRequestData.userInfo.id.toString() === _secret
        ) {
          return {
  ...
  async loginDeal(ctx, user) {
    ...
    if (app.config.verification_mode === 'jwt') {
      return user
        ? {
            accessToken: await ctx.helper.tools.apply(ctx, currentRequestData, app.config.jwt_exp),
            refreshToken: await ctx.helper.tools.apply(
              ctx,
              currentRequestData,
              app.config.jwt_refresh_exp,
              app.config.jwt.secret_refresh,
            ),
            csrf: ctx.csrf,
          }
        : null;
